...
*
* - javac --enable-preview --release 20 MinimizedStdLibTest.java
* - java --enable-preview MinimizedStdLibTest
*
...
public class MinimizedStdLibTest {
    ...
    public static void main(String[] args) {
        var allArgsForPrintf = printfArgs();

        for (int i = 0; i < allArgsForPrintf.length; i++) {
            var argsForPrintf = Arrays.stream(allArgsForPrintf[i])
                                      .map(PrintfArg.class::cast)
                                      .toList();
            try {
                test_printf(argsForPrintf);
                 Thread.sleep(1000);
            } catch (Throwable e) {
